    ...
    <section id="libros-cursos" className="py-12 md:py-20 bg-[#0a1120] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto flex flex-col gap-3 mb-12">
          <div className="inline-flex items-center gap-2 justify-center bg-amber-400/20 text-amber-300 border border-amber-400/40 px-3.5 py-1 rounded-full text-xs font-black w-fit mx-auto">
          ...
          {resources.map((r) => (
            ...
            >
              <div>
                ...
                {/* Title & Subtitle */}
                <h3 className="text-2xl font-black text-white mb-2 leading-snug group-hover:text-amber-400 transition-colors">
                  {r.title}
                </h3>
                <p className="text-xs font-extrabold text-white mb-4">{r.subtitle}</p>

                {/* Description */}
                <p className="text-sm text-white font-extrabold mb-6 leading-relaxed">{r.description}</p>

                {/* Bullet checklist */}
                <div className="space-y-3 mb-8 pt-4 border-t border-[#1e2a42]">
                  {r.features.map((f, idx) => (
                    <div key={idx} className="flex items-start gap-2.5 text-xs sm:text-sm text-white font-black">
              ...
              <div className="pt-4 border-t border-[#1e2a42]">
                <div className="flex items-baseline justify-between mb-4">
                  <span className="text-xs font-black text-white">Precio Especial:</span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="bg-[#1e2a42] hover:bg-[#28395a] text-white font-black py-3 px-3 rounded-xl transition text-xs flex items-center justify-center gap-1.5 border border-slate-700/60"
                  >
          ...
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-[#121b2d] border border-[#1e2a42] rounded-3xl max-w-2xl w-full max-h-[85vh] overflow-y-auto p-6 sm:p-8 relative text-white shadow-2xl">
              <button
                ...
                className="absolute top-4 right-4 bg-[#1e2a42] hover:bg-slate-700 text-white p-2 rounded-full transition"
              >
                <X className="w-6 h-6" />
              </button>
              ...
              <h3 className="text-2xl font-black text-white mb-2">{activePreview.title}</h3>
              <p className="text-xs font-black text-white mb-6">{activePreview.subtitle}</p>

              <div className="bg-[#0a1120] p-5 rounded-2xl border border-[#1e2a42] font-mono text-sm whitespace-pre-wrap text-white font-extrabold leading-relaxed mb-6">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-[#1e2a42]">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  <button
                    onClick={() => setActivePreview(null)}
                    className="flex-1 sm:flex-none px-4 py-2.5 rounded-xl border border-slate-700 text-white text-xs font-black"
                  >
                    Cerrar
                  </button>
                  <a
